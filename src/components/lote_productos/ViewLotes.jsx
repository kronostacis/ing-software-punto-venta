    ...
    <div className="fixed inset-0 bg-black bg-opacity-30 backdrop-blur-sm flex justify-center items-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            <label htmlFor="Id_producto" className="block text-sm font-medium text-gray-700">ID Producto</label>
            <input id="Id_producto" type="number" {...register("Id_producto")}
            ...
            />
            {errors.Id_producto && <p className="text-red-500 text-xs mt-1">{errors.Id_producto.message}</p>}
          </div>
          <div>
            <label htmlFor="Precio_compra" className="block text-sm font-medium text-gray-700">Precio de Compra</label>
            <input id="Precio_compra" type="number" step="0.01" {...register("Precio_compra")}
              className={`mt-1 block w-full px-3 py-2 border rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm ${
...
export default function ViewLotes() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <CreateLoteModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onLoteCreated={fetchLotes} />

      <div className="bg-white shadow-md rounded-lg overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ID Lote</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {lotes.map((lote) => (
              <tr key={lote.Id_lote} className="hover:bg-gray-100">
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{lote.Id_lote}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{lote.Id_producto}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${lote.Precio_compra.toLocaleString()}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{lote.Cantidad}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{lote.Stock}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{new Date(lote.Fecha).toLocaleDateString()}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-4">
                  <a href={`/lote_productos/${lote.Id_lote}`} className="text-indigo-600 hover:text-indigo-900">Editar</a>
                  <button onClick={() => handleDelete(lote.Id_lote)} className="text-red-600 hover:text-red-900">Eliminar</button>
                </td>
              </tr>
            ))}
          </tbody>
