...
function CreateMedioPagoModal({ isOpen, onClose, onMedioPagoCreated }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const msg = error.response?.data?.message || "Error al crear el medio de pago";
    ...
    <div className="fixed inset-0 bg-black bg-opacity-30 backdrop-blur-sm flex justify-center items-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            <label htmlFor="Nombre_pago" className="block text-sm font-medium text-gray-700">Nombre del Medio de Pago</label>
            <input id="Nombre_pago" type="text" {...register("Nombre_pago")}
            ...
            />
            {errors.Nombre_pago && <p className="text-red-500 text-xs mt-1">{errors.Nombre_pago.message}</p>}
          </div>
