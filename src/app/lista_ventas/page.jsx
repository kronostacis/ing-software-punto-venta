    ...
    <div className="fixed inset-0 bg-black bg-opacity-30 flex justify-center items-center z-50">
      <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-lg transform transition-all duration-300">
        <div className="flex justify-between items-center border-b pb-3 mb-4">
          <h2 className="text-2xl font-bold text-gray-800">Detalle de Venta #{venta.Id_venta}</h2>
          <button onClick={onClose} className="text-gray-500 hover:text-gray-800 text-2xl">&times;</button>
        </div>
        
        <div className="space-y-3 text-gray-700">
          <p><strong>Fecha:</strong> {new Date(venta.Fecha_venta).toLocaleString("es-CL", { dateStyle: "long", timeStyle: "short" })}</p>
          <p><strong>Total:</strong> <span className="font-semibold text-lg text-green-600">${venta.Total_venta.toLocaleString()}</span></p>
          <p><strong>Utilidad:</strong> <span className="font-semibold text-blue-600">${venta.Utilidad_total.toLocaleString()}</span></p>
          <p><strong>Atendido por:</strong> {venta.Usuarios.Nombre}</p>
          <p><strong>Medio de Pago:</strong> {venta.Medio_pagos.Nombre_pago}</p>
        </div>
            ...
            {Array.isArray(detalle) && detalle.length > 0 ? (
              detalle.map((item, i) => (
                <div key={i} className="flex justify-between items-center bg-gray-50 p-2 rounded-md">
                  <div>
                    <p className="font-semibold">{item.Productos?.Nombre || "N/A"}</p>
                    <p className="text-sm text-gray-500">Cantidad: {item.Cantidad}</p>
                  </div>
                  <p className="font-semibold text-gray-800">${item.Precio_total.toLocaleString()}</p>
                </div>
              ))
            ) : (
...
export default function ListaVentas() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
            ...
            >
              <div className="p-6">
                <div className="flex justify-between items-start">
                  <h2 className="text-xl font-bold text-gray-800">Venta #{venta.Id_venta}</h2>
                  <span
                ...
                <p className="text-sm text-gray-500 mt-1">
                  {new Date(venta.Fecha_venta).toLocaleString("es-CL", { dateStyle: "long", timeStyle: "short" })}
                </p>
                <div className="mt-4 space-y-2">
                  <p className="flex justify-between"><strong>Total:</strong> <span className="font-semibold">${venta.Total_venta.toLocaleString()}</span></p>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {ventas.map((venta) => (
                <tr key={venta.Id_venta} className="hover:bg-gray-100">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{new Date(venta.Fecha_venta).toLocaleString("es-CL", { dateStyle: "short", timeStyle: "short" })}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${venta.Total_venta.toLocaleString()}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{venta.Usuarios.Nombre}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{venta.Medio_pagos.Nombre_pago}</td>
      ...
      {isModalOpen && (
        <DetalleVentaModal venta={selectedVenta} detalle={detalle} onClose={closeModal} />
      )}
    </div>
  );
}
